...
describe('Sprint 3: OpenAI + Webhook Pipeline', () => {
  ...
  describe('OpenAI Service - Pre-flight Validation', () => {
    it('should reject solar data with missing annualProductionKwh', async () => {
      const invalidData = {
        building: mockSolarData.building,
        financial: {
    ...
    it('should reject solar data with missing savings20YearsRON', async () => {
      const invalidData = {
        ...mockSolarData,
        financial: {
          ...mockSolarData.financial,
      ...
      expect(result.status).toBe('insufficient_data');
      expect(result.missing).toContain('savings20YearsRON');
      console.log('[Test] ✓ Missing savings20YearsRON detected');
    });

    it('should reject solar data with missing systemCostRON', async () => {
      const invalidData = {
        ...mockSolarData,
        financial: {
          ...mockSolarData.financial,
      ...
      expect(result.status).toBe('insufficient_data');
      expect(result.missing).toContain('systemCostRON');
      console.log('[Test] ✓ Missing systemCostRON detected');
    });

    it('should reject solar data with missing governmentIncentiveRON', async () => {
      const invalidData = {
        building: mockSolarData.building,
        financial: {
          ...mockSolarData.financial,
          governmentIncentiveRON: 0,
        },
      };
      ...
      expect(result.status).toBe('insufficient_data');
      expect(result.missing).toContain('governmentIncentiveRON');
      console.log('[Test] ✓ Missing governmentIncentiveRON detected');
    });
